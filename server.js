...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const ok = file.mimetype.startsWith("image/") || file.mimetype.startsWith("audio/") || file.mimetype === "application/octet-stream";
    if (ok) cb(null, true);
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    for (const [cid, c] of calls) {
      if (c.from === userId || c.to === userId) {
        ...
        try { db.prepare("INSERT OR IGNORE INTO call_history VALUES (?,?,?,?,?,?,?,?)").run(cid, c.from, c.to, c.type, "ended", c.startedAt, Date.now(), dur); } catch(e){}
  ...
  socket.on("end_call", ({ callId }) => {
    ...
    if (!c) return;
    const dur = Math.round((Date.now() - c.startedAt) / 1000);
    try { db.prepare("INSERT OR IGNORE INTO call_history VALUES (?,?,?,?,?,?,?,?)").run(callId, c.from, c.to, c.type, "ended", c.startedAt, Date.now(), dur); } catch(e){}
    const other = c.from === userId ? c.to : c.from;
